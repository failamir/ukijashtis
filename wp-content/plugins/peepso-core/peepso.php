    ...
    private function __construct()
    {
        ...
        if (is_admin() && !PeepSo::is_api_request()) {
            ...
            add_action('peepso_config_after_save-site', function() {
                ...
                PeepSo3_Mayfly::del('monthly_license_warning');
            });
            ...
            add_action('admin_notices', function() {
                ...
                if ($current_month) {
                    $notice_content = PeepSo3_Mayfly::get('peepso_old_version_notice');
                    if (!$notice_content || isset($_GET['peepso_old_version_reset_html'])) {
                        $url = 'https://www.peepso.com/?product_old_version_warning';
                        $request = wp_remote_get($url, ['timeout' => 15, 'sslverify' => TRUE]);

                        if (is_wp_error($request)) {
                            $request = wp_remote_get($url, ['timeout' => 15, 'sslverify' => FALSE]);
                        }

                        if (!is_wp_error($request)) {
                            $notice_content = wp_remote_retrieve_body($request);
                            PeepSo3_Mayfly::set('peepso_old_version_notice', $notice_content, DAY_IN_SECONDS);
                        }
                    }
                }
...
//				$post = get_post($repost)
    ...
    public function check_ajax_query()
    {
        ...
        if ($segment >= 0 && 'peepsoajax' === $parts[$segment]) {
            ...
            if (array_key_exists('HTTP_REFERER', $_SERVER)) {
                setcookie('peepso_last_visited_page', $_SERVER['HTTP_REFERER'], time() + (MINUTE_IN_SECONDS * 30), '/', $_SERVER['HTTP_HOST'], isset($_SERVER['HTTPS']) ? true : false, true);
            }
    ...
    public function check_query()
    {
        ...
        if (isset($_GET['logout'])) {
            setcookie('peepso_last_visited_page', '', time() - 3600, '/', $_SERVER['HTTP_HOST'], isset($_SERVER['HTTPS']) ? true : false, true);
            wp_logout();
            ...
            setcookie('peepso_last_visited_page', $current_page, time() + (MINUTE_IN_SECONDS * 30), '/', $_SERVER['HTTP_HOST'], isset($_SERVER['HTTPS']) ? true : false, true);
...
//		}
    ...
    public function enqueue_scripts()
    {
        ...
        wp_register_style('peepso-fileupload',
            $logged_in ? PeepSo::get_asset('css/jquery.fileupload.min.css') : FALSE,
            NULL, PeepSo::PLUGIN_VERSION, 'all');

        wp_register_script('peepso-fileupload',
            $logged_in ? PeepSo::get_asset('js/fileupload.min.js') : FALSE,
        ...
        wp_register_script('peepso-crop', PeepSo::get_asset('js/crop.min.js'),
            $logged_in ? array('peepso', 'peepso-hammer') : array('peepso'),
            PeepSo::PLUGIN_VERSION, TRUE);

        wp_register_script('peepso-avatar-dialog',
            $logged_in ? PeepSo::get_asset('js/avatar-dialog.min.js') : FALSE,
            $logged_in ? array('peepso', 'peepso-avatar', 'peepso-crop') : NULL,
            PeepSo::PLUGIN_VERSION, TRUE);
...
// HTML, widget, linking utils
    ...
    */
    ...
    public function filter_peepso_navigation_profile($links)
    {
        ...
        if(PeepSo::is_dev_mode('profile_about_home')) {
            $links['stream'] = array(
                'href' => 'stream',
...
//        }
        ...
        $order = apply_filters('peepso_filter_navigation_profile_order', ['blogposts','followers','friends','groups','photos','media','videos','audio']);
        ...
        // #5989 let admin decide "about" to be first
        if(PeepSo::is_dev_mode('profile_about_home')) {
            $order = array_merge(array('about','stream'), $order);
        }
    ...
    public static function license_notice($plugin_name, $plugin_slug, $forced=FALSE)
    {
        ...
        switch ($license_data['response']) {
            case 'site_inactive':
                ...
                break;
            case 'expired':
                $message = 'License for PLUGIN_NAME has expired. The plugin will work, but it will not receive updates. You can renew your license <a target="_blank" href="PEEPSOCOM_LICENSES">here</a>.';

                if(45963636 == PeepSo3_Mayfly_Int::get('license_to_id')) {
                    $message = 'License for PLUGIN_NAME has expired. For monthly licenses the plugin will not work in the front-end or receive updates. You can renew your license <a target="_blank" href="PEEPSOCOM_LICENSES">here</a>.';
                }
