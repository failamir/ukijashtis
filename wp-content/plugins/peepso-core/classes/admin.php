...
//              $notice .= ' <a href="#TB_inline?inlineId=assign-roles-modal-id" class="thickbox">' . __('Click here', 'peepso-core') . '</a>' . __(' for more information on assigning roles.', 'peepso-core');
        ...
        if(PeepSo3_Mayfly::get('monthly_license_warning')) {
            ...
            $to = array(
                ...
                PeepSo::PEEPSOCOM_LICENSES,
            );

            $message = '
                    Your monthly PeepSo license has expired! The plugins will stop working in 48 hours after the expiry time!<br/>
                    Please <a target="_blank" href="PEEPSOCOM_LICENSES">renew your license or upgrade to a long term plan</a>.<br/>
                    <a target="_blank" href="PEEPSOCOM_CONTACT">Contact us</a> for more information.';
            $message = str_ireplace( $from, $to, $message );

            echo '<div class="error peepso" style="padding:11px 15px; margin:5px 15px 2px 0;"><b>' . $message . '</b></div>' . PHP_EOL;
        }
    ...
    public function admin_menu()
    {
        ...
        if(PeepSo3_Mayfly_Int::get('installer_has_new')) {
...
//	}
            ...
            foreach ($addons as $addon) {
                ...
                if ($activation instanceof WP_Error) {
                    ...
                    foreach ($activation->errors as $error) {
                        $errors++;
                        echo "<li><i class='gcis gci-exclamation-triangle'></i> <code>$addon</code>: {$error[0]}</li>";
                    }
                    $debug .= ob_get_clean();
                    continue;
                }
    ...
    public static function addons() {
        ...
        if (PeepSo::is_admin() && !get_user_option('peepso_user_installer_tutorial', get_current_user_id())) {
