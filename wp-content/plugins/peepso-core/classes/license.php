...
class PeepSoLicense
{
    ...
    public static function check_license($plugin_edd, $plugin_slug, $is_admin = FALSE)
    {
        ...
        #5870 expired monthly licenses need to be treated as invalid
        if($license_data['response'] == 'expired') {
            $bundle_id = PeepSo3_Helper_Addons::license_to_id($license_data['license']);
            if($bundle_id == 45963636) {
                if(isset($license_data['expire']) && $license_data['expire'] != 0) {
                    $diff = strtotime($license_data['expire']) - time();

                    // Grace period of 3 days, add a special warning to the admin
//                    echo $license_data['expire'];
//                    echo "<br>";
//                    echo date('Y-m-d H:i:s',time());
//                    echo "<br>";
//                    echo strtotime($license_data['expire']);
//                    echo "<br>";
//                    echo time();
//                    echo $diff;
//                    echo "<br>";
//                    die();
                    if($diff > -72*3600 && $diff < 0) {
                        ...
                        PeepSo3_Mayfly::set('monthly_license_warning',1,3600);
                        return TRUE;
                    }
    ...
    public static function activate_license($plugin_slug, $plugin_edd)
    {
        ...
        if ( !is_object($validation_data) ) {
            ...
            if (!$do_request || $peepso_is_offline) {
                ...
                $validation_data->expires			= '2023-03-03 23:23:23';
