...
//			'post_date_gmt' => date('Y-m-d H:i:s'), // gmdate('Y-m-d H:i:s'),
        ...
        new PeepSo3_Activity_Notifications($id, $author);
...
//			);
			...
			if ($orig_post->post_type == PeepSoActivityStream::CPT_POST) {
				...
				PeepSoMailQueue::add_notification_new($owner_id, $data, $message, $args, 'user_comment','user_comment', PeepSoActivity::MODULE_ID);

                $i18n = __('commented on your post', 'peepso-core');
                $message = 'commented on your post';
                $args = ['peepso-core'];

				$note->add_notification_new($author_id, $owner_id, $message, $args, 'user_comment', self::MODULE_ID, $id, $act_id);
			} else {
				...
				PeepSoMailQueue::add_notification_new($owner_id, $data, $message, $args, 'user_reply_comment', 'stream_reply_comment', PeepSoActivity::MODULE_ID);
                ...
                $note->add_notification_new($author_id, $owner_id, $message, $args, 'stream_reply_comment', self::MODULE_ID, $id, $act_id);
			}
        ...
        while ($users->have_posts()) {
            ...
            $note->add_notification_new($author_id, $users->post->post_author, $message, $args, 'user_comment', self::MODULE_ID, $id, $act_id);

            // activity_followers fallback for previous commenters
            new PeepSo3_Activity_Notifications($post_id, $users->post->post_author);
        }
...
//			'posts_per_page' => 1,
	...
	public function get_posts($offset = NULL, $owner_id = NULL, $user_id = NULL, $paged = NULL, $pinned = FALSE, $limit = 1, $post_id = 0, $search = NULL, $search_mode = 'exact')
	{
		...
		if(1 == $this->_input->int('stream_filter_sort_by', 0)) {
...
//			$permalink = $wpdb->escape($permalink);
        ...
        // add ORDER BY clause
        if(isset($_REQUEST['stream_filter_sort_by']) && 1 == $_REQUEST['stream_filter_sort_by']) {
...
//        }
    ...
    public function get_comments_var($post_or_post_id)
    {
        ...
        if ($post->post_type == PeepSoActivityStream::CPT_POST && get_post_meta($post->ID, 'peepso_pinned', TRUE)) {
            $pinned = TRUE;
        } else {
            ...
            if (isset($root_post) && $root_post && get_post_meta($root_post->ID, 'peepso_pinned', TRUE)) {
                $pinned = TRUE;
            }
        }
