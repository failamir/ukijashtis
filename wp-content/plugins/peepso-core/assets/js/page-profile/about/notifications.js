...
const toggleEmailOpts = (show = true) => {
	...
	if (show) {
		$nofifHeader.parent('.ps-js-notification-option').show();
		$emailBtns.show();
		$emailChks.show();
	} else {
		$nofifHeader.parent('.ps-js-notification-option').hide();
		$emailBtns.hide();
		$emailChks.hide();
	}
};
...
const togglePairedOpt = checkbox => {
	let $self = $(checkbox);
	let $pair = $self.closest('.ps-preferences__checkbox').find('[type=checkbox]').not($self);
