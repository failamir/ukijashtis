...
import { modules } from 'peepso';
import { rest_url, site_url, search as searchData } from 'peepsodata';

const REST_URL = rest_url;
const SITE_URL = site_url;
const SHOW_IMAGES = +searchData.show_images;
const SHOW_EMPTY_SECTIONS = +searchData.show_empty_sections;
const TEXT_NO_RESULTS = searchData.text_no_results;
...
class PageSearch {
	...
	search() {
		let query = this.$query.val().trim(),
			...
			endpoint_id = `${endpoint}_shortcode_${this.uniqueId}`,
			params = { query },
			transport;
...
$(() => {
	$('.ps-js-page-search').each(function () {
		new PageSearch(this);
	});
});
