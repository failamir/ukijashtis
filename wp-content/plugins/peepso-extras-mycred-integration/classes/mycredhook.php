...
class myCREDHook extends myCRED_Hook {
    ...
    function like_peepso_content($data) {
        ...
        if ($this->has_entry('like_peepso_content_'.$module_id, $to_id, $from_id))
            return;

        // Execute
        $this->core->add_creds(
            'like_peepso_content_' . $module_id, $from_id, $this->prefs['like_peepso_content']['creds'], $this->prefs['like_peepso_content']['log'], $to_id, array('ref_type' => 'post'), $this->mycred_type
        );
    }
...
WHERE p.pho_post_id = ' . (1 * $post_id);
    ...
    public function preferences() {
        ...
        <!-- Creds for Add Like PeepSo Content -->
        <label for="<?php echo $this->field_id(array('like_peepso_content', 'creds')); ?>"
                class="subheader"><?php echo $this->core->template_tags_general(__('%plural% for Add Like PeepSo Content / Profile', 'mycred')); ?></label>
        <ol>
            <li>
