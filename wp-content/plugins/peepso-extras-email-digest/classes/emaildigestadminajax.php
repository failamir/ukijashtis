...
class PeepSoEmailDigestAdminAjax extends PeepSoAjaxCallback {
	...
	public function show_log(PeepSoAjaxResponse $resp) {
		...
		if (count($results) > 0 && $page <= 5) {
			...
			ob_start();
			if ($page == 1) {
				echo '<style>@media(min-width:1200px){.memberdiv{min-height:100px;}}@media(min-width: 992px) and (max-width: 1199px){.memberdiv{min-height:150px;}}@media(min-width: 768px) and (max-width: 991px){.memberdiv{min-height:150px;}}</style>';
			}
